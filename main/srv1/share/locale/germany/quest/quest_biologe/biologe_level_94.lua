...
quest biologe_level_94 begin
	...
	state stage_quest_start begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Seon-Pyeongs Forschung 2" begin
			...
			printQuestHeader(mob_name(QUEST_NPC))
			sayQuestText("Vielen Dank! Du findest sie bei den Setaou-Kämpfern, sowie den Setaou-Soldaten in der Grotte der Verbannung.")
	...
	state stage_1 begin
		when login or enter or letter begin
			send_letter("Auftrag: Juwel der Lebenskraft")
		end

		when button or info begin
			printQuestHeader()
			sayQuestText(string.format("Seon-Pyeong gab dir den Auftrag %d Juwelen der Lebenskraft zu sammeln. Du findest sie bei den Setaou-Kämpfern, sowie den Setaou-Soldaten in der Grotte der Verbannung.", BIO_ITEM_COUNT))

			printQuestInfo()
			sayQuestText(string.format("Sammel alle %d Juwelen der Lebenskraft.", BIO_ITEM_COUNT))
			
			printQuestInfo("Hinweis:")
			sayQuestText(string.format("Du hast derzeit %s abgegeben!", pc.getqf("collectCount")))
		end

		when kill with npc.get_race() == 2401 or npc.get_race() == 2411 begin
		...
		when QUEST_NPC.chat."Auftrag: Juwel der Lebenskraft" with pc.count_item(BIO_ITEM_1) > 0 begin
			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Juwel der Lebenskraft. Ich würde dir raten zu %s zu gehen. Er hat eine Belohnung für dich bereit.", BIO_ITEM_COUNT, mob_name(REWARD_NPC)))
						wait()
			...
			elseif pc.getqf("playerGotMoreChance") == 1 then
				if perc <= CHANCE_WITH_PUSH then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Juwelen der Lebenskraft. Ich würde dir raten zu %s zu gehen. Er hat eine Belohnung für dich bereit.", BIO_ITEM_COUNT, mob_name(REWARD_NPC)))
						wait()
	...
	state stage_quest_reward begin
		...
		when __TARGET__.target.click or REWARD_NPC.chat."Belohnung des Biologen" begin
			...
			say_reward("Max-TP: +2664 (Dauerhaft)")
			wait()

			affect.add_collect(apply.MAX_HP, 2664, 60*60*24*365*60)
			clear_letter()
