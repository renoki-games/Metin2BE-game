...
quest biologe_level_98 begin
	...
	state stage_quest_start begin
		when login or letter begin
			send_letter("Seon-Pyeongs Forschung 4")
			setTarget(QUEST_NPC)
		end

		when button or info begin
			printQuestHeader("Seon-Pyeongs Forschung 4")
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Seon-Pyeongs Forschung 4" begin
			deleteTarget(QUEST_NPC)

			printQuestHeader(mob_name(QUEST_NPC))
			sayQuestText(string.format("Hallo %s, Ich bräuchte ein weiteres Mal deine Hilfe, kann ich dir vertrauen? Ich suche Juwelen der Verteidigung.Sie sollen deine Verteidigung extrem erhöhen können. Ich benötige %d Stück davon.", pc.get_name(), BIO_ITEM_COUNT))
			...
			printQuestHeader(mob_name(QUEST_NPC))
			sayQuestText("Vielen Dank! Du findest sie bei den Setaou-Anführern, sowie den Setaou-Kommandanten in der Grotte der Verbannung.")
	...
	state stage_1 begin
		when login or enter or letter begin
			send_letter("Auftrag: Juwel der Verteidigung")
		end

		when button or info begin
			printQuestHeader()
			sayQuestText(string.format("Seon-Pyeong gab dir den Auftrag %d Juwelen der Verteidigung zu sammeln. Du findest sie bei den Setaou-Anführern, sowie den Setaou-Kommandanten in der Grotte der Verbannung.", BIO_ITEM_COUNT))

			printQuestInfo()
			sayQuestText(string.format("Sammel alle %d Juwelen der Verteidigung.", BIO_ITEM_COUNT))
			
			printQuestInfo("Hinweis:")
			sayQuestText(string.format("Du hast derzeit %s abgegeben!", pc.getqf("collectCount")))
		end

		when kill with npc.get_race() == 2404 or npc.get_race() == 2414 begin
		...
		when QUEST_NPC.chat."Auftrag: Juwel der Verteidigung" with pc.count_item(BIO_ITEM_1) > 0 begin
			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Juwel der Verteidigung. Ich würde dir raten zu %s zu gehen. Er hat eine Belohnung für dich bereit.", BIO_ITEM_COUNT, mob_name(REWARD_NPC)))
						wait()
			...
			elseif pc.getqf("playerGotMoreChance") == 1 then
				if perc <= CHANCE_WITH_PUSH then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Juwelen der Verteidigung. Ich würde dir raten zu %s zu gehen. Er hat eine Belohnung für dich bereit.", BIO_ITEM_COUNT, mob_name(REWARD_NPC)))
						wait()
