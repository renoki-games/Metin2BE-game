...
quest biologe_level_60 begin
	...
	state stage_quest_start begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Die Forschung des Biologen 4" begin
			...
			sayQuestText(string.format("Hallo %s, kannst du mir helfen? Ich habe die Forschungen mit den Dämonenandenken abgeschlossen. Der Berg Sohan war schon immer ein wunderbarer Ort. Ich brauche die Eiskugeln von dort. Hilfst du mir bei meinen Untersuchungen?", pc.get_name()))
			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Eiskugeln. Ich benötige jetzt außerdem noch Aurtumryus Seelenstein.", BIO_ITEM_COUNT))
						wait()
	...
	state stage_2 begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Aurtumryus Seelenstein" begin
			...
			if pc.count_item(BIO_SOULSTONE) > 0 then
				printQuestHeader(mob_name(QUEST_NPC))
				sayQuestText("Dank deiner Hilfe verstehe ich nun die Legende des Aurtumryus Seelensteins. Du hast hart gekämpft. Sprich mit Baek-Go, er wird dir helfen deine inneren Kräfte zu steigern.")
				wait()
