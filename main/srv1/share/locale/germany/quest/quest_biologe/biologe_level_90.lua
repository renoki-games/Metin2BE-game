...
quest biologe_level_90 begin
	...
	state stage_quest_start begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Die Forschung des Biologen 8" begin
			...
			sayQuestText(string.format("Hallo %s, kannst du mir helfen? Ich habe die Forschungen mit den roten Ästen abgeschlossen. Ich möchte, dass du diesmal die Anführer der Gebiete besiegst. Bei ihnen findest du Notizen der Anführer. Hilfst du mir bei meinen Untersuchungen?", pc.get_name()))
			...
			printQuestHeader(mob_name(QUEST_NPC))
			sayQuestText("Vielen Dank! Du findest sie bei allen Anführern.")
	...
	state stage_1 begin
		...
		when button or info begin
			...
			printQuestInfo()
			sayQuestText(string.format("Sammel alle %d Notizen der Anführer.", BIO_ITEM_COUNT))
			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Notizen. Ich benötige jetzt außerdem noch Seelenstein der Anführer.", BIO_ITEM_COUNT))
						wait()
