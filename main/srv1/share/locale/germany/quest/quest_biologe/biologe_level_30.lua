...
quest biologe_level_30 begin
	...
	state stage_quest_start begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Die Forschung des Biologen" begin
			...
			sayQuestText(string.format("Hallo %s, kannst du mir helfen? Ich studiere zahlreiches aus der Pflanzenwelt. Zurzeit arbeite ich an einer Legende, die besagt, dass ein Orkzahn härter als Diamant sein soll. Hilfst du mir bei meinen Untersuchungen?", pc.get_name()))
			...
			printQuestHeader(mob_name(QUEST_NPC))
			sayQuestText("Vielen Dank! Du findest sie bei den Orks im Tal von Seungryong.")
	...
	state stage_1 begin
		...
		when button or info begin
			...
			printQuestInfo()
			sayQuestText(string.format("Sammel alle %d Orkzähne.", BIO_ITEM_COUNT))
			
			printQuestInfo("Hinweis:")
			sayQuestText(string.format("Du hast derzeit %s abgegeben!", pc.getqf("collectCount")))
		end

		when QUEST_NPC.chat."Auftrag: Orkzähne" with pc.count_item(BIO_ITEM_1) > 0 begin
			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Orkzähne. Ich benötige jetzt außerdem noch den Jinunggyis Seelenstein.", BIO_ITEM_COUNT))
						wait()
	...
	state stage_2 begin
		...
		when button or info begin
			...
			printQuestInfo()
			sayQuestText("Du findest ihn bei allen Stolzen Orks.")
		end
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Jinunggyis Seelenstein" begin
			deleteTarget(QUEST_NPC)

			if pc.count_item(BIO_SOULSTONE) > 0 then
				printQuestHeader(mob_name(QUEST_NPC))
				sayQuestText("Dank deiner Hilfe verstehe ich nun die Legende des Jinunggyis Seelensteins. Du hast hart gekämpft. Sprich mit Baek-Go, er wird dir helfen deine inneren Kräfte zu steigern.")
				wait()
