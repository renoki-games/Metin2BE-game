...
quest biologe_level_50 begin
	...
	state stage_quest_start begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Die Forschung des Biologen 3" begin
			...
			sayQuestText(string.format("Hallo %s, kannst du mir helfen? Ich habe die Forschungen mit den Fluchbüchern abgeschlossen und langweile mich. Ich würde gerne die Dämonen und deren Dämonenandenken studieren. Hilfst du mir bei meinen Untersuchungen?", pc.get_name()))
			...
			printQuestHeader(mob_name(QUEST_NPC))
			sayQuestText("Vielen Dank! Du findest sie bei den Dämonen im Dämonenturm.")
	...
	state stage_1 begin
		...
		when button or info begin
			...
			printQuestInfo()
			sayQuestText(string.format("Sammel alle %d Dämonenandenken.", BIO_ITEM_COUNT))
			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Dämonenandenken. Ich benötige jetzt außerdem noch Sagyis Seelenstein.", BIO_ITEM_COUNT))
						wait()
	...
	state stage_2 begin
		...
		when button or info begin
			...
			printQuestInfo()
			sayQuestText("Du findest ihn bei den Dämonen im Dämonenturm.")
		end
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Sagyis Seelenstein" begin
			...
			if pc.count_item(BIO_SOULSTONE) > 0 then
				printQuestHeader(mob_name(QUEST_NPC))
				sayQuestText("Dank deiner Hilfe verstehe ich nun die Legende des Sagyis Seelensteins. Du hast hart gekämpft. Sprich mit Baek-Go, er wird dir helfen deine inneren Kräfte zu steigern.")
				wait()
