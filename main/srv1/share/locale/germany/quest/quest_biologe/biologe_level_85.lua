...
quest biologe_level_85 begin
	...
	state stage_quest_start begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Die Forschung des Biologen 7" begin
			...
			sayQuestText(string.format("Hallo %s, kannst du mir helfen? Ich habe die Forschungen mit den Tafeln abgeschlossen. Ich möchte, dass du in den Roten Wald reist und dort Rote Geisterbaum-Äste erbeutest. Hilfst du mir bei meinen Untersuchungen?", pc.get_name()))
			...
			printQuestHeader(mob_name(QUEST_NPC))
			sayQuestText("Vielen Dank! Du findest sie bei den Kreaturen im Roten Wald.")
	...
	state stage_1 begin
		...
		when button or info begin
			printQuestHeader()
			sayQuestText(string.format("Der Biologe gab dir den Auftrag %d Rote Geisterbaum-Äste zu sammeln. Du findest sie bei den Kreaturen im Roten Wald.", BIO_ITEM_COUNT))

			printQuestInfo()
			sayQuestText(string.format("Sammel alle %d Rote Geisterbaum-Äste.", BIO_ITEM_COUNT))
			
			printQuestInfo("Hinweis:")
			sayQuestText(string.format("Du hast derzeit %s abgegeben!", pc.getqf("collectCount")))
		end

		when kill with npc.get_race() >= 2311 and npc.get_race() <= 2315 begin
			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d roten Geisterbaumäste. Ich benötige jetzt außerdem noch Waldgeist Seelenstein.", BIO_ITEM_COUNT))
						wait()
	...
	state stage_2 begin
		...
		when button or info begin
			...
			printQuestInfo()
			sayQuestText("Du findest ihn bei den Kreaturen im Roten Wald.")
		end
	...
	state stage_quest_reward begin
		...
		when __TARGET__.target.click or REWARD_NPC.chat."Belohnung des Biologen" begin
			...
			say_reward("Schadensreduzierung gegen andere Spieler: +10% (Dauerhaft)")
