			...
			if pc.getqf("playerGotMoreChance") == 0 then
				if perc <= CHANCE then
					if getBioQuestCount() < NEEDED_ITEM_COUNT then
						...
						printQuestHeader(mob_name(QUEST_NPC))
						sayQuestText(string.format("Vielen Dank für die %d Fluchsammlungen. Ich benötige jetzt außerdem noch den Tempel Seelenstein.", BIO_ITEM_COUNT))
						wait()
	...
	state stage_2 begin
		...
		when __TARGET__.target.click or	QUEST_NPC.chat."Tempel Seelenstein" begin
			...
			if pc.count_item(BIO_SOULSTONE) > 0 then
				printQuestHeader(mob_name(QUEST_NPC))
				sayQuestText("Dank deiner Hilfe verstehe ich nun die Legende des Tempel Seelensteins. Du hast hart gekämpft. Sprich mit Baek-Go, er wird dir helfen deine inneren Kräfte zu steigern.")
				wait()
