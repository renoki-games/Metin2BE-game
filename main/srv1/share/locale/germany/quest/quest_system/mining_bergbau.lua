quest mining begin
	state start begin
		when 20015.chat.gameforge[get_language()].fisher._10_npcChat begin
			setskin(NOWINDOW)
			npc.open_shop()
		end
		...
		when 20015.take with item.vnum >= 29101 and item.vnum < 29110 and item.get_socket(0) != item.get_value(2) begin
			...
			say_title(gameforge[get_language()].mining._10_sayTitle)
			say(gameforge[get_language()].mining._20_say)
		end
		when 20015.take with item.vnum >= 29101 and item.vnum < 29110 and item.get_socket(0) == item.get_value(2) begin
			say_title(gameforge[get_language()].mining._10_sayTitle)

			say(string.format(gameforge[get_language()].mining._30_say,  item.get_level()))
			if item.get_value(3) == 100 then
			say_title(gameforge[get_language()].mining._10_sayTitle)
				say(gameforge[get_language()].mining._40_say)
			else
			say_title(gameforge[get_language()].mining._10_sayTitle)
				say(string.format(gameforge[get_language()].mining._50_say,  100-item.get_value(3)))
			end

			local s =  select(gameforge[get_language()].locale.guild.yes, gameforge[get_language()].locale.guild.no)
			if s == 2 then
			say_title(gameforge[get_language()].mining._10_sayTitle)
				say(gameforge[get_language()].mining._55_say)
				return
			end
			...
			say_title(gameforge[get_language()].mining._10_sayTitle)
				...
				say(gameforge[get_language()].mining._60_say)
			elseif f == 1 then 
			say_title(gameforge[get_language()].mining._10_sayTitle)
				say(gameforge[get_language()].mining._70_say)
			else 
			say_title(gameforge[get_language()].mining._10_sayTitle)
				say(gameforge[get_language()].mining._80_say)
			end
		end
