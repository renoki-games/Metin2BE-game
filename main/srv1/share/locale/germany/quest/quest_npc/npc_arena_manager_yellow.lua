define QUEST_NPC_YELLOW 11003

quest npc_arena_manager_yellow begin
	state start begin
		when QUEST_NPC_YELLOW.chat."Lösche die Event-Flag" with pc.is_gm() begin
			game.set_event_flag("arenaStatus_YELLOW", 0)
			return
		end

		when QUEST_NPC_YELLOW.chat."Duellarena Info" begin
		...
		when QUEST_NPC_YELLOW.chat."Zurück in die Arena" with pc.getqf("active_player_a") == 1 or pc.getf("npc_arena_manager_yellow", "active_player_b") == 1 begin
			printQuestHeader("Schlachtenverwalter:")
			...
			if s == 2 then 
				game.set_event_flag("arenaStatus_YELLOW", 0)
				pc.setf("npc_arena_manager_yellow", "round", 0)
				pc.setf("npc_arena_manager_yellow", "active_player_b", 0)
				pc.setf("npc_arena_manager_yellow", "active_player_a", 0)
				notice("Dein Gegner muss das Duell auch abbrechen.")
			end

			if s == 1 then
				npc_arena_manager_yellow.warpToArenaPlayer_1(3)
			end
		end

		when QUEST_NPC_YELLOW.chat."Duell abbrechen" with pc.getqf("active_player_a") == 1 or pc.getf("npc_arena_manager_yellow", "active_player_b") == 1 begin
			printQuestHeader("Schlachtenverwalter:")
		...
		when QUEST_NPC_YELLOW.chat."In die Duellarena!" with pc.get_level() >= 15 begin
			printQuestHeader("Schlachtenverwalter:")

			if game.get_event_flag("arenaStatus_YELLOW") == 1 then 
			...
			game.set_event_flag("arenaStatus_YELLOW", 1)
			pc.setqf("active_player_a", 1)

			local b = find_pc_by_name(selectName)

			npc_arena_manager_yellow.warpToArenaPlayer_1(1)

			pc.select(b)
			
			pc.setf("npc_arena_manager_yellow", "active_player_b", 1)
			npc_arena_manager_yellow.warpToArenaPlayer_1(2)
		end

		when kill with pc.getqf("active_player_a") == 1 or pc.getf("npc_arena_manager_yellow", "active_player_b") == 1 begin
			local vid = npc.get_vid()
			local a = pc.select(vid)
			local name = pc.get_name()
	
			pc.select(a)
			if pc.getf("npc_arena_manager_yellow", "round") >= 2 then
				...
				npc_arena_manager_yellow.warpToArenaPlayer_1(4)
				...
				pc.setf("npc_arena_manager_yellow", "round", pc.getf("npc_arena_manager_yellow", "round") + 1)
				notice(pc.name.." gewinnt eine Runde!")
				notice(string.format("%s hat %s Runden gewonnen.",pc.name , pc.getf("npc_arena_manager_yellow", "round")))
			end
		end
	end
	state __FUNCTIONS__ begin
		function warpToArenaPlayer_1(val)
			if val == 1 then
				pc.warp(70800, 164400)
			elseif val == 2 then
				pc.warp(72500, 164400)
			elseif val == 3 then
				pc.warp(71700, 164500)
			elseif val == 4 then
				pc.warp(71700, 165700)
			end
		end
	end
end