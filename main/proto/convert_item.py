...
def item_proto():
	...
	rows = cur.fetchall()
	out_file_itemproto = open("proto/item_proto.txt", "w")
	out_file_itemnames_de = open("proto/item_names_de.txt", "w")
	print "Converting item_proto..."
	item_proto_firstline = "ITEM\tITEM_NAME(K)\tITEM_TYPE\tSUB_TYPE\tWEIGHT\tSIZE\tANTI_FLAG\tFLAG\tITEM_WEAR\tIMMUNE\tGOLD\tSHOP_PRICE\tLIMIT_TYPE0\tLIMIT_VALUE0\tLIMIT_TYPE1\tLIMIT_VALUE1\tADDON_TYPE0\tADDON_VALUE0\tADDON_TYPE1\tADDON_VALUE1\tADDON_TYPE2\tADDON_VALUE2\tVALUE0\tVALUE1\tVALUE2\tVALUE3\tVALUE4\tVALUE5\tSOCKET0\tSOCKET1\tSOCKET2\tREFINE\tREFINESET\tMAGIC_PCT\tSPECULAR\tSOCKET\tATTU_ADD"
	#item_proto_firstline = "ITEM\tITEM_NAME(K)\tITEM_TYPE\tSUB_TYPE\tSIZE\tANTI_FLAG\tFLAG\tITEM_WEAR\tIMMUNE\t?? ???\t???? ? ?\tREFINE\tREFINESET\tMAGIC_PCT\tLIMIT_TYPE0\tLIMIT_VALUE0\tLIMIT_TYPE1\tLIMIT_VALUE1\tADDON_TYPE0\tADDON_VALUE0\tADDON_TYPE1\tADDON_VALUE1\tADDON_TYPE2\tADDON_VALUE2\tVALUE0\tVALUE1\tVALUE2\tVALUE3\tVALUE4\tVALUE5\tSpecular\tSOCKET\tATTU_ADDON"
	out_file_itemproto.write(item_proto_firstline + "\r\n")
	item_names_firstline = "VNUM\tLOCALE_NAME"
	out_file_itemnames_de.write(item_names_firstline + "\r\n")
	for row in rows:
		item_proto_line = "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
		row[0],							## Vnum
		row[1],							## name
		get_item_type(row[4], row[0]),
		get_item_subtype(row[4], row[5], row[0]), 
		row[6],							## Weight
		row[7],							## Size
		get_antiflag(row[8], row[0]), 
		get_flag(row[9], row[0]),
		get_wearflag(row[10], row[0]),
		get_immuneflag(row[11]),
		row[12], 						## Gold
		row[13], 						## Shop_Buy_price
		get_limittype(row[18]), 
		row[19], 
		get_limittype(row[20]), 
		row[21], 
		get_apply(row[22]), 
		row[23], 
		get_apply(row[24]), 
		...
		item_names_de_line = "%s\t%s" % (row[0], row[2])
		out_file_itemproto.write(item_proto_line + "\r\n")
		out_file_itemnames_de.write(item_names_de_line + "\r\n")
